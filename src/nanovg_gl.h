...
static int glnvg__renderCreate(void* uptr)
{
	...
	static const char* fillFragShader =
	   ...
	   "}\n"
        ...
        "    float colorMix = 1.0 - smoothstep(0.0, lineThickness, abs(uv.x) - lineThickness * 0.5);\n"
        "    vec4 icol = innerCol;\n"
        "    if (uv.y < 0.0) {\n"
        ...
        "        icol = mix(outerCol, icol, innerCap);\n"
        "    }\n"
        "    result = mix(outerCol, icol, smoothstep(0.33, 0.66, clamp(colorMix, 0.0, 1.0))) * strokeAlpha * scissor;"
        "}\n"
		"	if (type == 0) {			// Gradient\n"
