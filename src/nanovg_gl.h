...
static int glnvg__renderCreate(void* uptr)
{
	...
	static const char* fillFragShader =
		...
		"float strokeMask() {\n"
		"   float mask = min(1.0, (1.0-abs(ftcoord.x*2.0-1.0))*strokeMult) * min(1.0, ftcoord.y);\n"
        ...
        "           float outerD = clamp((sdroundrect(pt, extent + vec2(0.5f), radius) + 1.5 * 0.5) / 1.5, 0.0, 1.0);\n"
        "           float innerD = clamp((sdroundrect(pt, extent - vec2(0.5f), radius - 1.0f) + 1.5 * 0.5f) / 1.5f, 0.0, 1.0);\n"
        "           vec2 dx = dFdx(pt);\n"
        "           vec2 dy = dFdy(pt);\n"
        "           float alias = 0.5 * length(max(abs(dx), abs(dy)));\n"
        "           float outerRoundedRectAlpha = 1.0f - (smoothstep(0.0, alias, outerD) * smoothstep(1.0 - alias, 1.0, outerD))\n;"
        "           float innerRoundedRectAlpha = 1.0f - (smoothstep(0.0, alias, innerD) * smoothstep(1.0 - alias, 1.0, innerD));\n"
