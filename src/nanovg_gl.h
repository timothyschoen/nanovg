...
static int glnvg__renderCreate(void* uptr)
{
	...
	static const char* fillFragShader =
        ...
        "           float outerD = clamp((sdroundrect(pt, extent + vec2(0.5), radius) + 1.5 * 0.5) / 1.5, 0.0, 1.0);\n"
        "           float innerD = clamp((sdroundrect(pt, extent - vec2(0.5), radius - 1.0) + 1.5 * 0.5) / 1.5f, 0.0, 1.0);\n"
        "           vec2 dx = dFdx(pt);\n"
        "           vec2 dy = dFdy(pt);\n"
