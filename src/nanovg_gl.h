...
static int glnvg__renderCreate(void* uptr)
{
	...
	static const char* fillFragShader =
	   ...
	   "}\n"
        ...
        "    float colorMix = 1.0 - 2.0 * abs(uv.x);\n"
        "    vec4 icol = innerCol;\n"
        "    vec4 ocol = outerCol;\n"
        "    float outerCap = 1.0;\n"
        "    if (uv.y < 0.5) {\n"
        ...
        "    }\n"
        "    if (uv.y > (lineLength - 0.5)) {\n"
        "        vec2 capStart = vec2(uv.x, lineLength - uv.y);\n"
        "        float dist = distance(vec2(1.0 - 2.0 * capStart.x, 1.0 - 2.0 * capStart.y), vec2(1.0, 0.0));\n"
        "        outerCap = 1.0 - step(1.0, dist);\n"
        "        float innerCap = 1.0 - smoothstep(0.33, 0.66, dist - 0.1);\n"
        "        icol = mix(ocol, icol, innerCap);\n"
        "    }\n"
        "    result = mix(ocol, icol, smoothstep(0.33, 0.66, clamp(colorMix, 0.0, 1.0))) * strokeAlpha * scissor * outerCap;\n"
        "}\n"
		"	if (type == 0) {			// Gradient\n"
