...
void nvgTransformGetSubpixelOffset(NVGcontext* ctx, float* tx, float* ty)
{
    ...
    float scaleX = std::sqrt(state->xform[0] * state->xform[0] + state->xform[1] * state->xform[1]);
    float scaleY = std::sqrt(state->xform[2] * state->xform[2] + state->xform[3] * state->xform[3]);

    // Extract translation
    float transX = state->xform[4] * scaleX;
    float transY = state->xform[5] * scaleY;

    // Snap translation to nearest device pixel
    float gridSize = 1.0f / ctx->devicePxRatio;
    float snappedX = std::round(transX / gridSize) * gridSize;
    float snappedY = std::round(transY / gridSize) * gridSize;

    *tx = state->xform[4] - snappedX / scaleX;
    *ty = state->xform[5] - snappedY / scaleY ;
}
